...
import re
# TODO:
# [V]  XVecs only for true_count > -2 (or some other threshold)
...
random.shuffle(learn_vec_args)  # for the lol


def getPKLkey(decks, penetration, spread):
    return f'decks_{decks}_penetration_{round(decks * penetration, 2)}_spread_{spread}'.replace(".", "_")

def initVectorW(decks, penetration, spread):
    return loadFromDB(getPKLkey(decks, penetration, spread), 'data/w_vectors.pkl')
...
class CountAgent:
    def __init__(self, needMatrixes, vec=False, spread=common.spread, penetration=common.penetration,
                 kellyFrac=common.vec_kelly_fraction, decks=common.num_of_decks, w_vec=None):
        ...
        if w_vec is None:
            self.w = initVectorW(self.game.shoe.n, self.game.shoe.penetration, self.spread)
        else:
            self.w = w_vec
...
def linear_reg(countAgent: CountAgent, pkl_key=None):
    ...
    common.w = np.concatenate((np.array([1000 * bias]), 1000 * np.array(w[1:])))
...
def singleGame(args):
    (i, vec, betMethod, decks, spread, penetration, kellyFrac, w_vec) = args
    ...
    while failed:
        countAgent = CountAgent(needMatrixes=False, vec=vec, spread=spread, penetration=penetration,
                                kellyFrac=kellyFrac, decks=decks, w_vec=w_vec)
        hands = 0
        ...
        while hands < common.graphs_max_hands:
            ...
            countAgent.runLoop(hands, betMethod, needCountDict=False)  # i=0 is redundant, just to pass something
            if hands % common.graphs_sample_rate == 0:
                data[hands] = countAgent.game.money
            # data[hands//common.graphs_sample_rate] = countAgent.game.money
            hands += 1

    return (i, data, countAgent.game.total_bets / ((common.graphs_max_hands - 1) * common.min_bet))


def multyProcessBatchGames(vec=False, betMethod='kelly', decks=common.num_of_decks, spread=common.spread,
                           penetration=common.penetration, kellyFrac=common.vec_kelly_fraction, queue=None, w_vec=None):
    ...
    avg_bet_sum = 0
    args = [(i, vec, betMethod, decks, spread, penetration, kellyFrac, w_vec) for i in range(common.graphs_num_of_runs)]
...
def MPBatchGamesWithArgs(vec, betMethod, penetration=common.penetration, kellyFrac=common.vec_kelly_fraction,
                         spread=common.spread, w_vec=None, queue=None, x=None):
    data, _ = multyProcessBatchGames(vec=vec, betMethod=betMethod, penetration=penetration, kellyFrac=kellyFrac,
                                     spread=spread, w_vec=w_vec)
...
def handleMPArgs(testedArg, x_indices, betMethod='kelly', vec=True, queue=None):
    ...
    w_vec = initVectorW(common.num_of_decks, common.penetration, common.spread)
    # Set the common arguments for the functions
    args = [vec, betMethod, common.penetration, common.vec_kelly_fraction if vec else common.hilo_kelly_fraction,
            common.spread, w_vec, queue2]
...
def batchGamesAndPenetrate(betMethod='kelly'):
    ...
    x_indices = dict()
    x_indices['vec'] = np.arange(1, common.num_of_decks * pen_rate - 2*pen_rate, -pen_rate)
    x_indices['hilo'] = np.arange(1, common.num_of_decks * pen_rate - 2*pen_rate, -pen_rate)
...
def kellyFractionGraph():
    ...
    x_indices = dict()
    x_indices['vec'] = np.arange(1.2, 1.7, common.kelly_frac_sample_rate)
    x_indices['hilo'] = np.arange(1.2, 1.7, common.kelly_frac_sample_rate)
...
def learnVecPerArg():
    ...
    for i, (decks, penetration, spread) in enumerate(learn_vec_args):
        pkl_key = f'decks_{decks}_penetration_{round(decks * penetration, 2)}_spread_{spread}'.replace(".", "_")
...
def findBestArgs():
    ...
    for (pkl_key, vec_advg, vec_mean, vec_std, hilo_mean, hilo_std) in tqdm(
            pool.imap_unordered(runBatchAndGetEdge, learn_vec_args), total=len(learn_vec_args)):
        vec_advg_dict[pkl_key] = vec_advg
        print(f'{pkl_key}\t\t{vec_advg}')

        spreadPlot(vec_mean=vec_mean, vec_std=vec_std, hilo_mean=hilo_mean, hilo_std=hilo_std,
                   file=f'spread_graphs/STDSpreadGraph_{pkl_key}')
    ...
    parts = best_pkl_key.split("_")
    common.num_of_decks = int(parts[1])
    common.penetration = round(float(parts[3] + '.' + parts[4]) / common.num_of_decks, 2)
    common.spread = int(parts[6])

    print(f'Validate: decks={common.num_of_decks}\tpenetration={common.penetration}\tspread={common.spread}')

def runBatchAndGetEdge(args):
    (decks, penetration, spread, pkl_key) = args
    vecData, vec_avg_bet_sum = multyProcessBatchGames(vec=True, betMethod='spread', decks=decks,
                                                      penetration=penetration, spread=spread)
    hiloData, hilo_avg_bet_sum = multyProcessBatchGames(vec=False, betMethod='spread', decks=decks,
                                                        penetration=penetration, spread=spread)
